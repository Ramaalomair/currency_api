...
async def recognize_with_image(file: UploadFile = File(...)):
    """
    يرجع صورة PNG تحتوي على الصورة الأصلية مع اسم العملة
    والنسبة المئوية للثقة مكتوبَين فوقها.
    """
    try:
        contents = await file.read()
        image = Image.open(io.BytesIO(contents)).convert("RGB")
...
# ──────────────────────────────────────────────
#  /recognize-multiple  →  عدة عملات (JSON)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
#  /recognize-multiple-with-image  →  عدة عملات (صورة PNG)
# ──────────────────────────────────────────────
@app.post("/recognize-multiple-with-image")
async def recognize_multiple_with_image(file: UploadFile = File(...)):
    """
    يرجع صورة PNG تحتوي على الصورة الأصلية مع مستطيلات
    حول كل ورقة نقدية مُكتشَفة واسمها فوقها، والمجموع الكلي.
    """
    try:
        contents = await file.read()
        image = Image.open(io.BytesIO(contents)).convert("RGB")
        ...
        colors = ["#00C853", "#2979FF", "#FF6D00", "#D500F9",
                  "#00BCD4", "#FF1744", "#AEEA00"]

        currencies_found = []
        total_value = 0

        if not detections:
            # محاولة التعرف على الصورة كاملة
            features = extract_features(image)
            prediction = svm_model.predict([features])[0]
            probabilities = svm_model.predict_proba([features])[0]
            confidence = float(probabilities[prediction]) * 100
            ...
            if confidence >= 40:
                ...
                value = int(currency_name.replace(" SR", "")) if currency_name != "Unknown" else 0
                total_value = value
                currencies_found.append(currency_name)
                ax.set_title(
                    f"{currency_name}  ({confidence:.1f}%)\nالمجموع: {total_value} SAR",
                    fontsize=14, color="green", fontweight="bold", pad=10
                )
            else:
                ax.set_title("لم يتم اكتشاف أي عملة", fontsize=14,
                             color="red", fontweight="bold", pad=10)
        else:
            for idx, det in enumerate(detections):
                x1, y1, x2, y2 = det["bbox"]
                ...
                # كتابة اسم العملة فوق المستطيل
                ax.text(
                    x1, y1 - 8,
                    f"{currency_name} ({confidence:.0f}%)",
                    color="white", fontsize=11, fontweight="bold",
                    bbox=dict(facecolor=color, alpha=0.85, pad=3, edgecolor="none")
                )

            ax.set_title(
                f"عدد الأوراق: {len(currencies_found)}  |  المجموع: {total_value} SAR",
                fontsize=14, color="#1565C0", fontweight="bold", pad=12
            )
        ...
        logger.error(f"recognize-multiple-with-image error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
